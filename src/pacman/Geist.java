...
public class Geist
{
    ...
    {
        controle = 0;
        sprite = new ImageIcon("Geist.png").getImage();
        this.x = x;
        this.y = y;
    }
    

    public int Geistmover(int pacx, int pacy, boolean power)
    {   
        controle++;
        if(power==true){
        if (controle == 2){
        if (pacx < getX() ){dir = 1;}
        else if (pacx > getX()){dir = 4;}}
        if (controle == 4){
        if (pacy < getY()){dir = 2;}
        else if (pacy > getY()){dir = 3;}
        controle = 0;
        }}else{
        if (controle == 2){
        if (pacx < getX() ){dir = 4;}
        else if (pacx > getX()){dir = 1;}}
//        if (controle == 4){
//        dir = new Random().nextInt((4-1)+1)+1;}
        if (controle == 4){
        if (pacy < getY()){dir = 3;}
        else if (pacy > getY()){dir = 2;}
        controle = 0;
        }}
        return 0;
    }
